...
class CQLPolicy(Policy):
    ...
    def _init_learn(self) -> None:
        ...
        if self._cfg.learn.get('adaptive_in_dataset_penalty',None):
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        # if self._cfg.learn.get('local_q_target', None):
        #     q_target=default_collate(data).get('q_target')
        #     if self._cuda:
        #         q_target = to_device(q_target, self._device)
        ...
        if self._value_network:
            ...
            with torch.no_grad():
                ...
                if self._cfg.learn.get('learn_action_num_argmax', False):
                    action_num = self._cfg.collect.get('q_exploration_action_num', 1)
                    pred = dist.rsample((action_num,)).reshape(-1,mu.shape[-1])
                    next_action = torch.tanh(pred)
                    collect_data = {'obs': next_obs.unsqueeze(0).repeat(action_num,1,1).reshape(-1,next_obs.shape[-1]), 'action': next_action}
                    tem_q_value = self._learn_model.forward(collect_data, mode='compute_critic')['q_value'][0]
                    tem_q_value = tem_q_value.reshape(action_num,-1)
                    next_action = next_action.reshape(action_num,*mu.shape)
                    pred = pred.reshape(action_num,*mu.shape)
                    index=tem_q_value.argmax(dim=0).reshape(1,-1,1).repeat(1,1,next_action.shape[-1])
                    next_action = next_action.gather(dim=0,index=index).squeeze(0)
                    pred = pred.gather(dim=0,index=index).squeeze(0)
                else:
                    pred = dist.rsample()
                    next_action = torch.tanh(pred)
        ...
        if not self._cfg.learn.online:
            ...
            if self._cfg.learn.get('min_q_weight_td_error', None):
                if self._cfg.learn.min_q_weight_td_error_type=='identity':
                    ...
                    if flag:     
                        ...
                        elif self._cfg.learn.get('local_q_target', None):
                            min_q_weight_meta = self.min_q_weight_meta[0] + (q_value[0].detach()-data['q_target'])
                            self.min_q_weight_meta[0] += (q_value[0].detach()-data['q_target']).mean().item()
                        else:
                        ...
                        if self._cfg.learn.get('local_q_target', None):
                            min_q_weight_meta=torch.clamp(min_q_weight_meta, min=meta_min)
                            self.min_q_weight=min_q_weight_meta
                else:
                    raise NotImplementedError            
            min_qf1_loss = (torch.logsumexp(cat_q1, dim=1,) * self.min_q_weight).mean()
            min_qf2_loss = (torch.logsumexp(cat_q2, dim=1,) * self.min_q_weight).mean()
            ...
            if self._cfg.learn.get('min_q_weight_on_all_penalty',None):
                ...
                min_qf1_loss = min_qf1_loss - (iod*q_value[0]).mean()*notin_ratio
                min_qf2_loss = min_qf2_loss - (iod*q_value[1]).mean()*notin_ratio
            else:
                min_qf1_loss = min_qf1_loss - ((iod*q_value[0]) * self.min_q_weight).mean()
                min_qf2_loss = min_qf2_loss - ((iod*q_value[1]) * self.min_q_weight).mean()
            ...
            if self._cfg.learn.get('min_q_weight_td_error', None):
                if self._cfg.learn.min_q_weight_td_error_type=='q_value_inverse':
                    beta = 2.5/(q_value[0].detach().mean().item())
                elif self._cfg.learn.min_q_weight_td_error_type=='percentage':
                    beta = (loss_dict['critic_loss'].detach().mean().item())/(min_qf1_loss.detach().mean().item()+1e-7)*self._cfg.learn.min_q_weight_td_error_percentage
                else:
                    beta=1
            else:
                beta=1
            loss_dict['critic_loss'] += min_qf1_loss*beta
            if self._twin_critic:
                loss_dict['twin_critic_loss'] += min_qf2_loss*beta
        ...
        if self._cfg.learn.get('learn_action_num_argmax', False):
            ...
            collect_data = {'obs': data['obs'].unsqueeze(0).repeat(action_num,1,1).reshape(-1,data['obs'].shape[-1]), 'action': action}
            tem_q_value = self._learn_model.forward(collect_data, mode='compute_critic')['q_value'][0]
            tem_q_value = tem_q_value.reshape(action_num,-1)
            action = action.reshape(action_num,*mu.shape)
            pred = pred.reshape(action_num,*mu.shape)
            index=tem_q_value.argmax(dim=0).reshape(1,-1,1).repeat(1,1,action.shape[-1])
            action = action.gather(dim=0,index=index).squeeze(0)
            pred = pred.gather(dim=0,index=index).squeeze(0)
        else:
            pred = dist.rsample()
            action = torch.tanh(pred)
        ...
        try:
            ret['min_q_weight'] = float(self.min_q_weight.detach().mean().item())
        except Exception:
            ret['min_q_weight'] = float(self.min_q_weight)
        if not self._cfg.learn.get('online', False):
            ret['min_qf1_loss'] = min_qf1_loss.detach().mean().item()
            ret['min_qf2_loss'] = min_qf2_loss.detach().mean().item()
            ret['std_q1'] = std_q1.detach().mean().item()
        if self._cfg.learn.get('min_q_weight_td_error_type',None)=='mean_scale':
    ...
    def _forward_collect(self, data: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            dist = Independent(Normal(mu, self._cfg.collect.get('sigma_ratio', 1)*sigma), 1)
            action_num = self._cfg.collect.get('q_exploration_action_num', 1)
            q_exploration_type = self._cfg.collect.get('q_exploration_type', 'sample')
            if q_exploration_type == 'noise':
                base = torch.tanh(dist.rsample())
                noise = torch.randn((action_num,mu.shape[-1]), device=mu.device) * self._cfg.collect.get('q_exploration_noise_sigma', 0.1) + 0
                action = base + noise 
                action = action.clamp(-1, 1)
            elif q_exploration_type == 'sample':
                action = torch.tanh(dist.rsample((action_num,)).squeeze(1))
            data=data.repeat(action_num,1)
            collect_data = {'obs': data, 'action': action}
            q_value = self._learn_model.forward(collect_data, mode='compute_critic')['q_value'][0]
            action = action[q_value.argmax()].unsqueeze(0)
            output = {'logit': (mu, sigma), 'action': action, 'sample_mu_dist': F.pairwise_distance(action, torch.tanh(mu), p=2)}
        if self._cuda:
