...
class SACPolicy(Policy):
    ...
    def _init_learn(self) -> None:
        ...
        if self._cfg.learn.get("sphere_optim", None):
            self.q_fc_layer=[m for m in self._model.critic.modules() if isinstance(m, torch.nn.Linear)]
            self.init_q_fc_norm=self._compute_norm([l.weight.clone().detach() for l in self.q_fc_layer])
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        if self._value_network:
            ...
            with torch.no_grad():
                ...
                if self._twin_critic:
                    ...
                    # target_q_value = torch.cat([i.unsqueeze(-1) for i in target_q_value], dim=-1) - self._alpha * next_log_prob
                else:
        ...
        if self._twin_critic:
            ...
            # q_value=torch.cat([i.unsqueeze(-1) for i in q_value], dim=-1)
            # td_error_per_sample = torch.nn.MSELoss(reduction='none')(q_value, reward.view(-1,1) + self._gamma * target_q_value)
            # td_error_per_sample = td_error_per_sample.mean()
            # loss_dict['critic_loss'] = td_error_per_sample
            # loss_dict['twin_critic_loss']=0.0
        else:
        ...
        # sphere optimization
        if self._cfg.learn.get("sphere_optim", None):
            curr_weight_norm = self._compute_norm([l.weight.clone().detach() for l in self.q_fc_layer])
            for l in self.q_fc_layer:
                l.weight.data.mul_(self.init_q_fc_norm / curr_weight_norm)
